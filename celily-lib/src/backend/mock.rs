...
impl super::InstanceBackend for MockInstanceBackend {
    ...
    fn delete(&self, name: &str) -> Result<(), Self::Error> {
        self.calls.lock().unwrap().push(MockInstanceCall::Delete {
            name: name.to_string(),
        });
        Ok(())
    }
    ...
    fn add_device(&self, name: &str, dev_name: &str, _device: &Device) -> Result<(), Self::Error> {
        self.calls
            .lock()
            .unwrap()
            .push(MockInstanceCall::AddDevice {
                name: name.to_string(),
    ...
    fn attach_to_bridge(
        ...
        self.calls
            .lock()
            .unwrap()
            .push(MockInstanceCall::AttachToBridge {
                name: name.to_string(),
    ...
    fn set_description(&self, name: &str, _desc: &str) -> Result<(), Self::Error> {
        self.calls
            .lock()
            .unwrap()
            .push(MockInstanceCall::SetDescription {
                name: name.to_string(),
            });
        Ok(())
    }
    ...
    fn exec(
        ...
        self.calls.lock().unwrap().push(MockInstanceCall::Exec {
            name: name.to_string(),
        });
        Ok(0)
    }
    ...
    fn exec_stdout(&self, name: &str, _cmd: &[&str]) -> Result<String, Self::Error> {
        self.calls
            .lock()
            .unwrap()
            .push(MockInstanceCall::ExecStdout {
                name: name.to_string(),
            });
    ...
    fn write_file(
        ...
        self.calls
            .lock()
            .unwrap()
            .push(MockInstanceCall::WriteFile {
                name: name.to_string(),
                path: path.to_string(),
            });
        Ok(())
...
impl super::NetworkBackend for MockNetworkBackend {
    ...
    fn create_bridge(
        ...
        self.calls
            .lock()
            .unwrap()
            .push(MockNetworkCall::CreateBridge {
                name: name.to_string(),
            });
